...
const HouseCard = ({ rental }) => {
  ...
  return (
    <div>
      <p className="text-center mb-1">{rental.name}</p>
      <div
      ...
      >
        ...
        <div className="mt-2 text-sm w-full px-2">
          ...
          <div className="font-medium  grid grid-cols-2 gap-1">
            <p>Location:</p>
            <p className="text-right min-w-fit text-ellipsis">
              {rental.address}
            </p>
          </div>
          {/* region */}
          ...
          <div className="text-gray-500 grid grid-cols-2 gap-1">
            <p>Down payment:</p>
            <p className="text-right min-w-fit text-ellipsis">{(rental.price * 12) * .20}</p>
          </div>
        </div>
      </div>
    </div>
